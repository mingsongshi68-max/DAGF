"""
here is the mian backbone for DAGF
"""
import torch
...
class DAGF(nn.Module):
    def __init__(self, args):
        ...
        # lightweight shared gate for cross-modal robustness
        self.fusion_gate = nn.Sequential(
            nn.Linear(self.d_l * 3, self.d_l * 3),
            nn.Sigmoid()
        )
    ...
    def forward(self, text, audio, video):
        ...
        # adaptive gating across modalities
        gate_input = torch.cat([last_h_l, last_h_v, last_h_a], dim=1)
        gate = self.fusion_gate(gate_input)
        gate_l, gate_v, gate_a = gate.chunk(3, dim=1)
        last_h_l = last_h_l * (0.5 + gate_l)
        last_h_v = last_h_v * (0.5 + gate_v)
        last_h_a = last_h_a * (0.5 + gate_a)
        ...
        # 回归头输出，并用 tanh 将范围约束在 [-3, 3] 区间，提升 MOSI 上 acc_7/acc_5/Corr/MAE 等指标的稳定性
        output = self.out_layer(last_hs_proj)
        output = 3 * torch.tanh(output)
